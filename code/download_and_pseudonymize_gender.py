...
def write_posts_users(dirPath, pseudonymized_data):
    ...
    with open (dirPath + 'posts_german.csv', 'w', newline='', encoding="utf-8") as postFile:
    ...
    for key in pseudonymized_data['participants'][0]:
        fieldnames.append(key)
    with open (dirPath + 'participants_german.csv', 'w', newline='', encoding="utf-8") as partFile:
...
def write_anno_codes(anno, codes):
    ...
    for key in anno[0]:
        fieldnames.append(key)
    with open (dirPath + 'annotations_german.csv', 'w', newline='', encoding="utf-8") as annoFile:
        annoWriter = csv.DictWriter(annoFile, fieldnames = fieldnames)
        annoWriter.writeheader()
        for annotation in anno:
            annotation['language'] = 'german'
            annoWriter.writerow(annotation)
    # codes:
    fieldnames = []
    for key in codes[0]:
        fieldnames.append(key)
    with open (dirPath + 'codes_german.csv', 'w', newline='', encoding="utf-8") as codesFile:
...
if __name__ == '__main__':
    ...
    gendermap =  complete_gender_info(dirPath + 'polish_genders.csv')
    ## change the tag variable to the tag that denotes your project
    tag = 'ethno-rebelpop-deutsch-interviews'
